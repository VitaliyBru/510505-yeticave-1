<main>
    <?= $nav_panel; ?>
    <form class="form container<?= $errors['form'] ? ' form--invalid' : ''; ?>" action="login.php" method="post">
        <!-- form--invalid -->
        <h2>Вход</h2>
        <?php if ($errors['email']['isWrong'] || $errors['password']['isWrong']): ?>
            <div class="form__item form__item--invalid">
                <span class="form__error"
                      style="font-size:16px;">Вы ввели неверный<?= $errors['email']['isWrong'] ? ' email' : ' пароль'; ?></span>
            </div>
        <?php endif; ?>
        <div class="form__item<?= ($errors['email']['isEmpty'] or $errors['email']['isWrong']) ? ' form__item--invalid' : ''; ?>">
            <!-- form__item--invalid -->
            <label for="email">E-mail*</label>
            <input id="email" type="text" name="login[email]" value="<?= strip_tags($login['email']); ?>"
